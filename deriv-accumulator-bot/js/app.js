...
class App {
  ...
  init() {
    ...
    if (this.wsClient.apiToken) {
      ...
      this.log('info', 'Welcome! Enter your App ID & API Token to connect.');
    }
  }

  loadSavedCredentials() {
    const defaultAppId = '345cWwqRPJnpWl7nnZGpu';
    const defaultToken = 'pat_48d08b86206ed6ccd4878d92c12d414a527071c671ffe0d85d9709d389237327';
    ...
    if (!savedToken || savedToken.trim().length === 0 || savedToken.includes('90b1eef3')) {
      savedToken = defaultToken;
  ...
  bindDOMEvents() {
    ...
    document.getElementById('btnSaveApiCredentials').addEventListener('click', () => {
      ...
      if (!appId || appId.length === 0) {
        this.log('error', 'App ID cannot be empty.');
        return;
      }
      if (!token || token.length === 0) {
        this.log('error', 'API Token cannot be empty.');
        return;
      }
      ...
      document.getElementById('apiModal').classList.remove('active');
      this.log('success', `API credentials saved (App ID: ${appId}).`);
    ...
    const bindNumericInput = (id, key, isFloat = false) => {
