...
export class AccumulatorBotEngine {
  ...
  start() {
    ...
    // Initiate symbol subscription & first trade loop
    this.client.subscribeTicks(this.config.symbol)
      .then(() => {
        this.triggerNextTrade();
      })
      .catch(err => {
        this.emit('log', { type: 'error', message: `Failed to subscribe to ticks: ${err.message}` });
